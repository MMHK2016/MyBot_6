...
Func getRemainTrainTime($Troops = True, $Spells = True)
	...
	Local $ResultSpells = getOcrSpellsTime(363, 423)

    SetLog(" Total time train troop(s): " & $ResultTroops)
    SetLog(" Total time brew Spell(s): " & $ResultSpells)

	If $Troops = True Then
		If StringInStr($ResultTroops, "h") > 1 Then ; eg. 1h2m
		...
		Else
			If StringInStr($ResultTroops, "m") > 1 Then ; eg. 23m
				$aRemainTrainTroopTimer = Number(StringTrimRight($ResultTroops, 1))
			Else
				$aRemainTrainTroopTimer = 1 ; eg. 20s treat as 1m
			EndIf
		EndIf
	EndIf
